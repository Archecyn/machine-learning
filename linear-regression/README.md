...
In linear regression, the goal is to find the best-fit line that minimises the difference between the predicted values and the actual values. The best-fit line is determined by estimating the coefficients or weights associated with each independent variable.
...
The coefficients β0 and β1 are estimated using a method called ordinary least squares (OLS), which minimises the sum of squared errors.
